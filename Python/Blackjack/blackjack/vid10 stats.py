...
def newhand(cardsLeft, money):
    ...
    original_true = true_count

    if true_count >= 1:
        bet = 1#0*betSize*true_count
    else:
        bet = betSize
    ...
    if takeInsurance:
        if dealer[1][:-1] in ['10', 'J', 'K', 'Q']:
            ...
            money -= bet/2
    if original_true <= 10 and original_true >= -5:
        true_money_graph[original_true + 5].append(money-money_initial)
        if money > money_initial:
            wins_true[original_true+5] += 1
        elif money < money_initial:
            loss_true[original_true+5] += 1
...
repetitions = 240000000
betSize = 1

true_money_graph = [[] for i in range(16)]
wins_true = [0 for i in range(16)]
loss_true = [0 for i in range(16)]
...
while rep <= repetitions: #and money > 0:
    rep += 1
    if len(cardsLeft) <= (decks * 52 * 0.25):
        cardsLeft = reshuffle(decks)
    cardsLeft, money = newhand(cardsLeft,money)

for j in range(len(true_money_graph)):
    graphY = [(100000+true_money_graph[j][0]) for i in range(len(true_money_graph[j]))]
    for i in range(1,len(true_money_graph[j])):
        graphY[i] = graphY[i-1] + true_money_graph[j][i]
    print ('For',j-5)
    print (len(true_money_graph[j]),'hands')
    print(graphY[-1]-100000, 'dollars')
    x_values = np.arange(len(graphY))
    #plt.plot(x_values,graphY)
    #plt.ylabel('Money ($)')
    #plt.xlabel('Hands Played')
    #plt.title(('True Count:' + str(j - 5)))
    #plt.show() 
    #winsPercent = int(10000*wins_true[j]/(wins_true[j]+loss_true[j])) / 100
    #plt.bar((('Wins\n' + str(winsPercent)+'%'),('Loss\n'+str(100-winsPercent)+'%')),(wins_true[j],loss_true[j]))
