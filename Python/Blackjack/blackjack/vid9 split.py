...
def newhand(cardsLeft, money):
    global money_graph, wins, loss, betSize, count, prepared, winnings, move, truth
    blackjack = False
    ...
    if not dealer_blackjack and not blackjack:
        ...
        while splitting:
            ...
            for i in range(len(players)):
                valuation, soft = value([players[i][0]])
                if value([players[i][0]]) == value([players[i][1]]):
                    true_count = trueCount(cardsLeft)
                    if true_count == truth:
                        location = list(true_split_strategy).index((valuation, dealer[0][:-1], 'N'))
                        split = random.choice(['yes','no'])
                        move[location] = split
    ...
    prepared = [False for i in range(len(true_split_strategy))]
    move = ['no' for i in range(len(true_split_strategy))]
...
winnings = [[0,0,0,0] for i in range(len(true_split_strategy))]
prepared = [False for i in range(len(true_split_strategy))]
move = ['no' for i in range(len(true_split_strategy))]
truth = 5
...
for i in range(len(winnings)):
    print("For the combination " + str((list(true_split_strategy))[i]) + ' we get:')
    if winnings[i][2] != 0 and winnings[i][3] != 0:
        yesResult = winnings[i][0]/winnings[i][2]
        noResult = winnings[i][1]/winnings[i][3]
        if noResult > yesResult:
            print('The best is to not split with £' + str(noResult) + ' being made from gathering ' + str(winnings[i][2]) + " results!")
        else:
            print('The best is to split with £' + str(noResult) + ' being made from gathering ' + str(winnings[i][3]) + " results!")
    else:
