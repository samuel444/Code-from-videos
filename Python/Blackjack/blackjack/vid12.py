...
def round_sig_np(x, sig=2):
    if x == 0:
        return 0
    return np.round(x, -int(np.floor(np.log10(abs(x))) - (sig - 1)))
...
def boundariesFunc(money):
    for i in range(len(boundaries)):
        if money > boundaries[i] and money <= boundaries[i+1]:
...
def findBet(money):
    ...
    for i in range(20):
        text.append(text[15])
    if money > 3000000:
        text = ['500', '500', '500', '500', '500', '500', '2500', '3500', '5500', '10500', '14000', '18500', '23000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000', '25000']
    return text   
...
boundaries = [int(round_sig_np(1000*(1.3**i))) for i in range(100)]

money_bound, prev = boundariesFunc(money)
betSize = findBet(money_bound)
...
while rep <= repetitions and money > 0:
    rep += 1
    if len(cardsLeft) <= (decks * 52 * 0.2) or trueCount(cardsLeft) < -3:
        cardsLeft = reshuffle(decks)
    if money > boundaries[prev+1] or money < boundaries[prev-2]:
        money_bound, prev = boundariesFunc(money)
        betSize = findBet(money_bound)
    cardsLeft, money = newhand(cardsLeft,money)


x_values = np.arange(len(money_graph))/200
